...
describe('scoreJob', () => {
  ...
  it('scales with staleness', () => {
    const fresh    = scoreJob({ status: 'active', lastStatusChange: ago(1) })
    const stale2   = scoreJob({ status: 'active', lastStatusChange: ago(2) })
    const stale3   = scoreJob({ status: 'active', lastStatusChange: ago(3) })
    const stale7   = scoreJob({ status: 'active', lastStatusChange: ago(7) })
    expect(fresh).toBe(0)
    expect(stale2).toBeGreaterThan(fresh)
    expect(stale3).toBeGreaterThan(stale2)
    expect(stale7).toBeGreaterThan(stale3)
  })
  it('adds points for high progress', () => {
    const low  = scoreJob({ status: 'active', progress: 25 })
    const mid  = scoreJob({ status: 'active', progress: 60 })
    const high = scoreJob({ status: 'active', progress: 80 })
    expect(high).toBeGreaterThan(mid)
    expect(mid).toBeGreaterThan(low)
  })
  it('adds 40 for needs-action status', () => {
    const s = scoreJob({ status: 'needs-action' })
